function gpFit = infV1_gp2d_lin(X,Y)
...
% covFunc = {@covADD,{[1,2],'covSEiso','covLINone'}};
% hyp.cov = [log(std(X(:,1)));0;0;0;0];
covFunc_dist = {@covMask {[1 0], @covSEiso}};
covFunc_corr = {@covMask {[0 1], @covLINone}};
covFunc = {@covProd {covFunc_dist, covFunc_corr}};
hyp.cov =  [log(std(X(:,1)));0;0];
...
xu_2 = linspace(prctile(X(:,2),0.1),prctile(X(:,2),99.9),9)'; 
