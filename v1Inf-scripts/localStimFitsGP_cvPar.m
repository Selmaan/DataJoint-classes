function [oH,oPh,gPar,tR] = localStimFitsGP_cvPar(allResp,nNeurons)
...
Y = sqrt(allResp.Y(:,nNeurons)); Y = Y./repmat(std(Y),size(Y,1),1); 
...
for k = 1:kFolds
    ...
    parfor nNeuron = 1:size(Y,2)
        if sum(Y(:,nNeuron)) > 1
            thisHyp = minimize(hyp, @gp, -100, ...
                infFunc, meanFunc, covFunc, likFunc, xTrain, yTrain(:,nNeuron));
            [foldPreds(:,nNeuron),~,~,~,foldProbs(:,nNeuron)] = gp(thisHyp, infFunc, meanFunc, ...
...
for nNeuron = 1:size(Y,2)
    params.predCorr(nNeuron,:) = corr(Y(:,nNeuron),...
        [allPreds(:,nNeuron),trainPreds(:,nNeuron)]);
end
